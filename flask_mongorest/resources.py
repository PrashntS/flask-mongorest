...
class Resource(object, metaclass=ResourceMeta):
    ...
    def fetch_related_resources(self, objs, only_fields=None):
        ...
        for k,v in document_queryset.items():
            ...
            hint_index = {}
            if k in list(self.related_resources_hints.keys()):
        ...
        for obj in objs:
            for field, hint_index in hints.items():
                obj_id = obj.id
                if isinstance(obj_id, DBRef):
                    obj_id = obj_id.id
                elif isinstance(obj_id, ObjectId):
                    obj_id = str(obj_id)
                if obj_id not in list(hint_index.keys()):
                    setattr(obj, field, [])
                    continue
